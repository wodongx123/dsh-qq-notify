...
function apply(ctx, config) {
  ...
  async function drainBody(req) { for await (const _ of req) {} }
  ...
  ctx.effect(() => {
    const dispose = ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-qq-notify/status',
      handler: async (req, res) => {
        if (req.method !== 'GET' || !isLoopback(req.socket.remoteAddress)) {
          sendJson(res, req.method === 'GET' ? 403 : 405, { error: 'Request rejected.' });
          return;
        }
        // Check installation
        let napcatDir = cfg.napcatDir || '';
        if (!napcatDir || !isNapcatDir(napcatDir)) {
          for (const p of KNOWN_NAPCAT_PATHS) {
            if (isNapcatDir(p)) { napcatDir = p; break; }
          }
        }
        ...
        const napcatStatus = login ? 'running' : installed ? 'stopped' : 'notInstalled';

        sendJson(res, 200, {
          config: { mainQq: cfg.mainQq, apiPort: cfg.apiPort, napcatDir: cfg.napcatDir, botQq: cfg.botQq },
          napcatStatus,
          napcatDir,
          botUserId: login ? String(login.user_id) : null,
  ...
  ctx.effect(() => {
    const dispose = ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-qq-notify/config',
      handler: async (req, res) => {
        if (req.method !== 'POST' || !isLoopback(req.socket.remoteAddress)) {
  ...
  ctx.effect(() => {
    const dispose = ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        const output = await runQqPs(cfg.napcatDir, action);
  ...
  ctx.effect(() => {
    const dispose = ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-qq-notify/find',
      handler: async (req, res) => {
        if (req.method !== 'GET' || !isLoopback(req.socket.remoteAddress)) {
          sendJson(res, req.method === 'GET' ? 403 : 405, { error: 'Request rejected.' });
          return;
        }
        // Scan for NapCat instances (same logic as qq_find tool)
        const rootDir = cfg.napcatDir ? dirname(dirname(cfg.napcatDir)) : '';
        ...
        for (const sd of scans) {
          try {
            const found = scanForNapcat(sd, 6);
            for (const r of found) {
              if (!seen.has(r.dir)) { seen.add(r.dir); allResults.push(r); }
            }
          } catch {}
        }
        for (const kp of KNOWN_NAPCAT_PATHS) {
          if (isNapcatDir(kp) && !seen.has(kp)) { seen.add(kp); allResults.push({ dir: kp, exact: true }); }
        }
    ...
    const dispose = ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        if (!Number.isSafeInteger(to) || to <= 0) {
          sendJson(res, 400, { error: 'Invalid mainQq configuration.' });
          return;
        }
        try {
          const resp = await fetch(`${apiBase(cfg)}/send_private_msg`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ user_id: to, message: '[dsh-qq-notify] 这是一条测试消息 ✓' }),
            signal: AbortSignal.timeout(10000)
          });
          ...
          if (data.status === 'ok') {
            ...
            sendJson(res, 200, { ok: false, message: data.wording || data.message || 'Send failed.' });
          }
        } catch (err) {
          sendJson(res, 200, { ok: false, message: `NapCat API unreachable: ${err?.message ?? err}` });
        }
      }
    });
    return dispose;
